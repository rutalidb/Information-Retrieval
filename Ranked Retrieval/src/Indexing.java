...
class Indexing{
	...
	public Map<String, Integer> termIndex = new TreeMap<String,Integer>();
	public static TreeMap<String, Dictionary> invertedDictionary = new TreeMap<String, Dictionary>();
	public TreeMap<String, Dictionary> invertedStemDictionary = new TreeMap<String, Dictionary>();
	public static StanfordLemmatizer objlemma =  new StanfordLemmatizer();
	...
	public static void main(String args[]) throws Exception
	{
		...
		Indexing.uncompressedDictionary(uncompressedIndex, uncompressedPostings,i.invertedDictionary);
		endTimer = System.currentTimeMillis();
		System.out.println("The elapsed time required to build uncompressed Lemma Index: " + (endTimer - startTimer) + " milliseconds");
		
		startTimer = System.currentTimeMillis();
		Indexing.uncompressedDictionary(uncompressedStemIndex, uncompressedStemPostings,i.invertedStemDictionary);
		endTimer = System.currentTimeMillis();
		System.out.println("The elapsed time required to build uncompressed Stem Index: " + (endTimer - startTimer) + " milliseconds");
		
		
		IndexCompression.createCompressedDictionary(i.invertedDictionary);
		...
		IndexCompression.createCompressedStemDictionary(i.invertedStemDictionary);
		...
		System.out.println("The number of inverted lists in version 1 uncompressed of the index: " + i.invertedDictionary.size());
		...
		System.out.println("The number of inverted lists in version 2 uncompressed of the index: " + i.invertedStemDictionary.size());
		...
		System.out.println("Document Frequency: " + i.invertedDictionary.get("nasa").docFrequency);
		System.out.println("TermFrequency: " + i.invertedDictionary.get("nasa").termFrequency);
		
		computeNASATerms("NASA");
		System.out.println("-------------------------------------");
		System.out.println("Index 1:");
		largestLowestdf(i.invertedDictionary);
		System.out.println("-------------------------------------");
		System.out.println("Index 2:");
		largestLowestdf(i.invertedStemDictionary);
		System.out.println("-------------------------------------");
	...
	public static void getDfTfLength(String term) {
			...
			System.out.print(i.invertedDictionary.get(term).docFrequency + "\t");
			System.out.print(i.invertedDictionary.get(term).termFrequency + "\t");
	...
	public static void computeNASATerms(String term) {
		Indexing id = new Indexing();
		term = objlemma.lemmatize(term.toLowerCase());
		int i = 0;
		List<PostingList> list = id.invertedDictionary.get(term).postings;
	...
	public static void documentMaxLenDocLen() {
        ...
        for (Entry<String, Dictionary> entry : i.invertedStemDictionary.entrySet()) {
	...
	public static void createStopWordsSet(File stopWords) throws IOException {
		QueryProcess.stopWordlist = new HashSet<String>();
		Scanner scanner = new Scanner(stopWords);
		while(scanner.hasNext()){
			QueryProcess.stopWordlist.add(scanner.next());
		}
		scanner.close();
	}
	
	public void buildIndex(String directoryPath) throws Exception{
		...
		for (File f : files) {			
			...
			totalDoclen += t.substringArrayLength;
	...
	public static void addToIndex(int docID, Map<String, Integer> termIndex, TreeMap<String, Dictionary> DesiredDictionary, int doclen) {
		...
		//if not a stop word, then add to inverted file
			...
			if(!QueryProcess.stopWordlist.contains(term)) {
